...
SELECT EMP_NAME "직원 명", EMAIL "이메일", PHONE "전화번호", HIRE_DATE "고용일"
FROM EMPLOYEE;
...
SELECT EMP_NAME "이름", DEPT_CODE "부서 코드", HIRE_DATE "고용일"
FROM EMPLOYEE
WHERE DEPT_CODE IN ('D9','D5') AND HIRE_DATE < TO_DATE('20020101');
...
WHERE 
    ...
    HIRE_DATE BETWEEN'90/01/01' AND '01/01/01' AND 
    SALARY >= 2700000;
...
SELECT EMP_NAME, ABS(FLOOR(HIRE_DATE - SYSDATE)) AS "근무일수1", ABS(FLOOR(SYSDATE - HIRE_DATE)) AS "근무일수2"
FROM EMPLOYEE;
...
SELECT *
FROM EMPLOYEE
WHERE ABS(FLOOR(SYSDATE - HIRE_DATE)) >= 20 AND ENT_DATE IS NULL;

--20. EMPLOYEE 테이블에서 사원명, 급여 조회 (단, 급여는 '\9,000,000' 형식으로 표시)
SELECT EMP_NAME "사원 명", TO_CHAR(SALARY,'L9,000,000') "급여"
FROM EMPLOYEE;
...
SELECT 
    EMP_NAME, DEPT_CODE, TO_CHAR(TO_DATE(SUBSTR(EMP_NO,1,6)),'YY"년"MM"월"DD"일"') "생일",
    --  EXTRACT (YEAR FROM SYSDATE) - EXTRACT (YEAR FROM TO_DATE(SUBSTR(EMP_NO,1,2),'RRRR')) "나이(만)"
    TRUNC(MONTHS_BETWEEN(SYSDATE , TO_DATE(SUBSTR(EMP_NO,1,6),'RRMMDD'))/12) "나이(만)"
FROM EMPLOYEE;
...
-- 주민번호 앞자리와 뒷자리의 합 조회
SELECT EMP_NAME, SUBSTR(EMP_NO,1,6) AS "주민번호 앞자리", SUBSTR(EMP_NO,8,6) AS "주민번호 뒷자리", SUBSTR(EMP_NO,1,6) + SUBSTR(EMP_NO,8,6)
FROM EMPLOYEE;

--24. EMPLOYEE테이블에서 부서코드가 D5인 직원의 보너스 포함 연봉 합 조회
SELECT SUM((SALARY + (SALARY * BONUS)) * 12) "연봉 합"
FROM EMPLOYEE
GROUP BY DEPT_CODE
HAVING DEPT_CODE = 'D5';
...
--SELECT  COUNT(*) AS "전체 직원 수" , TO_CHAR(TO_DATE(SUBSTR(HIRE_DATE,1,2),'RRRR'),'RRRR"년"') AS "년도"
--FROM EMPLOYEE
--GROUP BY SUBSTR(HIRE_DATE,1,2)
--ORDER BY TO_CHAR(TO_DATE(SUBSTR(HIRE_DATE,1,2),'RRRR'),'RRRR');
SELECT
    COUNT(*),
    COUNT(BONUS), 
