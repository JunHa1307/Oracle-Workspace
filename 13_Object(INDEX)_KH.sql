/*
    <INDEX>
    전체적인 DEMS성능향상을 위한 설정
    
    책에서 '목차' 같은 것
    
    만약 목차가 없다면, 내가 원하는 페이지, 챕터가 어디에 있는지 책을 하나하나 훑어 봐야함
    
    마찬가지로 테이블에서 JOIN 혹은 서브쿼리로 데이터를 조회할 때
    인덱스가 없다면 테이블의 모든 데이터를 하나하나 비교하면서(FULL SCAN) 내가 원하는 데이터를 가져올 것
    
    따라서 인덱스 설정을 해두면 테이블의 모든 행에 접근하지 않고도 내가 원하는 조건만 빠르게 가져올수 있다
...
--  현재 계정에 생성된 인덱스들 확인
SELECT* FROM USER_INDEXES; -- PK 설정시 자동으로 인덱스 생성됨
...
WHERE EMP_NAME = '선동일'; -- CARDINALITY 10 / 25
--  우리가 생성한 그 인덱스를 활용할지 말지는 옵티마이저가 판단을 하게됨
...
/*
    인덱스를 효율적으로 쓰기 위한 방법
    데이터의 분포도가 높고, 조건절에 자주 호출되며, 중복값이 적은 칼럼 => PK칼럼
    ...
    2) ORDER BY 연산을 사용할 필요가 없다(이미 정렬되어 있기 때문)
    
    3) MIN, MAX등의 집계함수로 값을 찾을 때 연산속도가 매우 빠름 (정렬되어있기 때문에)
    
    인덱스의 단점
    1) DML에 취약함
        INSERTM UPDATE, DELETE등 데이터가 새롭게 추가, 삭제가 되면 인덱스 테이블 안에 있는 값들도 다시 정렬하고,
        ROWID(물리적 주소)값도 수정해줘야함
