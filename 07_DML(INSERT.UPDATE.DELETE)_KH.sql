...
/*
    ...
    1) INSERT INTO 테이블면 VALUES(값,값2,값3,...);
...
INSERT INTO EMPLOYEE
VALUES (901, '홍길동', '001213-3155674','hong@kh.or.kr','01099999999','D1','J7','S6','2200000',NULL,NULL,SYSDATE
...
/*
    2) INSERT INTO 테이블명(컬럼명1,컬럼명2,컬럼명3,...) 
       ...
       -    그래도 한 행 단위로 추가되기 때문에 선택이 안된 칼럼은 기본적으로 NULL OR DEFAULT값이 들어감
       
       주의사항 : NOT NULL 제약조건 혹은 PRIMARY KEY 제약조건이 걸려있는 칼럼은 반드시 직접 값을 넣어줘야한다
                예외사항으로 NOT NUILL + DEFAULT는 예외
*/
...
/*
    ...
    => VALUES()로 값을 직접 기술하는 것이 아니라 서브쿼리로 조회한 결과값을 통째로 INSERT하는 구문
    여러행을 한 번에 INSERT 할 수 있음
*/
...
(
    SELECT 2 AS BOARD_IMAGE_NO, 'abcD.jpg' AS ORIGIN_NAME, '2022122012345.jpg' AS CHANGE_NAME
    FROM DUAL
    UNION
    SELECT 3 AS BOARD_IMAGE_NO, 'abcF.jpg' AS ORIGIN_NAME, '2022122012345.jpg' AS CHANGE_NAME
    FROM DUAL
    UNION
    SELECT 4 AS BOARD_IMAGE_NO, 'abcG.jpg' AS ORIGIN_NAME, '2022122012345.jpg' AS CHANGE_NAME
    FROM DUAL
);
...
/*
    ...
    SET 칼럼명 = 바꿀 값,
        칼럼명 = 바꿀 값,
        칼렴명 = 바꿀 값, -- 여러개의 값을 동시에 변경 가능 (,로 변경할 값들을 나열해야함 / AND아님)
        ...
...
--  전체 사원의 급여를 기존  급여에 25% 인상해주기 기존급여 * 1.25, 기존급여 + 기존급여 *0.25
UPDATE EMP_SALARY
...
/*
    ...
    -   CREATE시에 서브쿼리 사용 : 서브쿼리를 수행한 결과를 테이블 만들 때 넣어버리겠다
...
--  EMP_SALARY테이블에 홍길동사원의 부서코드를 선동일 사원의 부서코드로 변경
--  홍길동 부서코드 D1, 선동일 부서코드 D9
...
--  DELETE문으로 EMPLOYEE테이블 안의 홍길동, 민경민 정보를 지우기
DELETE FROM EMPLOYEE
WHERE EMP_NAME IN ('홍길동','민경민');

--  WHERE 절의 조건에 따라 1개 이상의 행 OR 0개 행이 변경 될 수 있다
COMMIT;

--  DEPARTMENT테이블로부터 DEPT_ID가 D1인 부서 삭제
DELETE FROM DEPARTMENT
WHERE DEPT_ID = 'D1';
--  만약에 EMPLOYEE테이블의 DEPT_CODE 컬럼에서 외래키로 DEPT_ID칼럼을 참조하고 있을 경우
--  삭제가 되지 않았을 것.
...
/*
    ...
    TRUNCATE TABLE 테이블명;          ㅣ            DELETE FROM 테이블명;
    ========================================================================
