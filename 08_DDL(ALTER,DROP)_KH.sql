/*
    ...
    -수정할 내용
    1) 칼럼 추가 / 수정 / 삭제
    2) 제약조건 추가 / 삭제 -> 수정은 불가 (수정하고자 한다면 삭제후 새롭게 추가)
    3) 테이블 명 / 칼럼 명 / 제약조건 명 수정
    
*/
--  1) 칼럼추가/ 수정/ 삭제
--  1_1) 칼럼추가 (ADD) : ADD 추가할 칼럼명 자료형 DEFAULT 기본값 (단, DEFAULT 기본값은 생략가능)
...
--  DEFAULT지정해서 LNAME 칼럼 추가
ALTER TABLE DEPT_COPY ADD LNAME VARCHAR2(20) DEFAULT '한국';
--  새로운 칼럼이 만들어지고, NULL값이 아닌 DEFAULT값으로 채워짐

--  1_2) 칼럼 수정(MODIFY)
--       컬럼의 자료형 수정 : MODIFY 수정할 컬럼명 바꾸고자 하는 자료형
--       DEFAULT값 구정 : MODIFY 수정할 컬럼명 DEFAULT 바꾸고자 하는 기본값

--  DEPT_COPY 테이블로 DEPT_ID컬럼의 자료형을 CHAR(3)으로 변경
ALTER TABLE DEPT_COPY MODIFY DEPT_ID CHAR(3);

--  현재 변경하고자 하는 컬럼에 이미 문자열로 담겨있는 값을 완전 다른 타입으로 변경이 불가능함.
ALTER TABLE DEPT_COPY MODIFY DEPT_ID NUMBER;

--  현재 변경하고자 하는 칼럼에 담겨있는 값보다 "작게"는 변경이 불가능하다
ALTER TABLE DEPT_COPY MODIFY DEPT_ID CHAR(1);

--  한번에 여러개의 컬럼 변경 가능
--  DEPT_TITLE칼럼의 데이터 타입을 VARCHAR2(40)
...
--  DML문만 롤백 가능 : DDL문은 롤백이 안됨 <= 자동 커밋되기 때문
ROLLBACK;
...
/*
    ...
    -   FORRIGN KEY : ADD FOREIGN KEY(컬럼명) REFERENCES 참조할 테이블[(참조할 칼럼)]
    ...
    -> CONSTRAINT 제약조건명은 생략 가능하다(임의의 이름이 붙음)
    -> 주의사항 : 현재 계정 내에 고유한 이름으로 부여해야함
*/
--  DEPT_COPY테이블로부터
--  DEPT_ID칼럼에 PRIMARY KEY제약조건 추가
...
--  DEPT_COPY 칼럼명에 DCOPY_PK 제약조건 지우기
ALTER TABLE DEPT_COPY DROP CONSTRAINT DCOPY_PK;

--  DEPT_COPY테이블에 DCOPU_UQ, DCOPY_NN 제약조건 지우기
ALTER TABLE DEPT_COPY 
...
--  3_2) 제약조건명 변경 : RENAME CONSTRAINT 기존 제약명 TO 바꿀 제약명
ALTER TABLE DEPT_COPY RENAME CONSTRAINT SYS_C0011241 TO DCOPY_DIDNN;

--  3_3) 테이블명 변경 : RENAME 기존 테이블명 TO 바꿀 테이블명 || RENAME 바꿀 테이블명
...
/*
    ...
    객체를 삭제하는 구문
    [표현법]
    DROP 객체 [TABLE, USER, VIEW 등등] 삭제하고자 하는 객체이름;
...
--  이 때 부모 테이블은 DEPT_TEST테이블의 DEPT_ID를 참조
...
--  부모테이블 삭제하고싶다면
--  방법1) 자식 테이블을 먼제 삭제하고 그 후에 부모 테이블을 삭제
DROP TABLE EMPLOYEE_COPY3;
DROP TABLE DEPT_TEST;

--  방법2) 부모테이블만 삭제하되 맞물려있는 외래 키 제약조건도 함께 삭제해준다
