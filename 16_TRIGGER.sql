/*
    ...
    EX)
    회원탈퇴시 기존의 회원테이블에 데이터 DELETE후곧바로  탈퇴된 회원들만 따로 보관하는 테이블에 자동으로 INSERT처리해야된다.
    신고횟수가 일정 수를 넘었을때 묵시적으로 해당 회원을블랙리스트 처리되게끔.
    입출고에 대한 데이터가 기록될때마다(INSERT될때마다) 해당상품에 대한 재고수량을 매번 수정(UPDATE)해야 될때 등.
    
    * 트리거 종류
    SQL문의 시행시기에 따른 분류
    > BEFORE TRIGGER : 내가 지정한 테이블에 이벤트(INSERT, UPDATE, DELETE)가 발생되기전에 트리거실행
    > AFTER  TRIGGER : 내가 지정한 테이블에 이벤트가 발생된 후에 트리거실행.
    
    SQL문에 의해 영향을 받는 각 행에 따른 분류
    > STATEMENT TRIGGER(문장 트리거) : 이벤트가 발생한 SQL문에 대해 딱 한번만 트리거 실행
    > ROW TRIGGER(행 트리거) : 해당 SQL문이 실행될때마다 매번 트리거 실행(FOR EACH ROW옵셜 기술해야됨)
    ...
    * 트리거 생성구문
    ...
    DECLARE
        변수선언;
    BEGIN
        실행내용(해당 위에 지정한 이벤트 발생시 자동으로 실행할구문)
    EXCEPTION
    END;
    /
*/
...
-- 이력번호로 매번 새로운번호를 발생시켜 들어갈수 있게 도와주는 시퀀스
...
-- TB_PRODUCT테이블에서 200번상품 10개를 증가시키는 UPDATE문 실행
UPDATE TB_PRODUCT
 SET STOCK = STOCK +10
WHERE PCODE = 200;
...
/*
    ...
    TB_PRODUCT테이블에 재고수량이 자동으로 UPDATE되게끔 트리거 만들기.
    
    - 상품이 입고된경우 -> 해당상품을 찾아서 재고수량을 증가 UPDATE +
    
    - 상품이 출고된경우 -> 해당상품을 찾아서 재고수량 감소 UPDTAE -
*/
...
BEGIN
    ...
    IF (:NEW.STATUS = '입고')
        THEN
        UPDATE TB_PRODUCT
        SET STOCK = STOCK + :NEW.AMOUNT -- NEW키워드활용해서 TB_PRODETAIL에 인서트된값을 활용할수 있다.
        WHERE PCODE = :NEW.PCODE;
    ELSE
        -- 상품이 출고된경우 -> 재고수량을 감소시켜줘야한다.
        UPDATE TB_PRODUCT
...
INSERT INTO TB_PRODETAIL
VALUES(SEQ_DCODE.NEXTVAL, 200, SYSDATE , 90 ,'출고');
...
/*
    ...
    트리거 단점
    1. 빈번한 추가, 수정, 삭제시 ROW의 삽입 ,추가 삭제가 함께실행되므로 성능상 좋지 못함.
    2. 관리적 측면에서 형상관리가 "불가능"하기때문에 관리에 불편하다
    3. 트리거를 남용하게되면 예상치 못한 사태가 발생할수 있으며 유지보수가 힘들다.
*/
