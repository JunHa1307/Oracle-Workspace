/*
    ...
    사원정보는 사원테이블, 직급정보는 직급 테이블, ... => 중복을 최소화하기 위함.
    => 즉, JOIN구문을 이용해서 여러개 테이블간의 관계를 맺어서 같이 조회해야함.
    => 단, 무작정 JOIN을 하는게 아니라 테이블간에 "연결고리(포린키)"에 해당되는 칼럼을 매칭시켜서 조회해야함.
    
    문법상 분류 : JOIN은 크게 오라클 전용구문과 ANSI구문으로 나누어짐.
    
            오라클 전용 구문                   ㅣ              ANSI구문(오라클 + 다른 DBMS에서 사용가능)
    ====================================================================================================
...
--  JOIN을 통해서 연결고리에 해당되는 컬럼들만 제대로 매칭시키면 마치 하나의 결과물로 조회 가능하다

/*
    1. 등가조인(EQUAL JOIN) / 내부조인(INNER JOIN)
    연결시키고자 하는 칼럼값들이 "일치하는 행들"만 조인해서 조회
    ( == 일치하지 않는 값들은 결과에서 제외)
    => 동등비교 연산자 = 일치한다라는 조건을 제시
    [표현법]
    등가조인(오라클구문)
    ...
    + 만약에 연결할 칼럼명이 동일하다면 USING구문 사용하고,
                        동일하지 않는다면 명시적으로 어느 테이블로부터 온 칼럼인지 작성해줘야 한다
*/
--  오라클 전용구문
...
WHERE DEPT_CODE = DEPT_ID;--    EMPLOYEE테이블의 DEPT_CODE값과 DEPARTMENT테이블의 DEPT_ID이 일치하는 경우에만 찾는다
--  일치하지 않는 값은 조회되지 않는다 (부서코드가 NULL인 사원)
--  두개 이상의 테이블을 조인할 때 일치하는 값이 없는 행은 결과에서 제외됨.

--  전체 사원들의 사번, 사원명, 직급코드, 직급명을 알아내고자 한다면
--  연결할 칼럼명이 일치(EMPLOYEE - JOB_CODE/ JOB - JOB_CODE)
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE, JOB
WHERE JOB_CODE = JOB_CODE; -- 에러발생 : COLUMN AMBIGUOUSLY DEFINED : 컬럼명이 애매모호하다 
                            --> 확실히 어디있는 칼럼인지를 제시해줘야함
...
--  전체 사원들의 사번, 사원명, 부서코드, 부서명을 알아내고자 할 때
--  1) 연결할 칼럼명이 다른경우 (EMPLOYEE - DEPT)_CODE / DEPARTMENT - DEPT_ID)
--      => 무조건  ON 구문만 가능.
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_TITLE
FROM EMPLOYEE
...
--  => ON, USING구문 둘 다 사용가능함
--  2-1) ON구문 : AMBIGUOUSLY가 발생할 수 있기 때문에 칼럼명을 명확하게 기술해줘야함
SELECT EMP_ID, EMP_NAME, E.JOB_CODE, JOB_NAME
FROM EMPLOYEE E
...
--  [참고(잘안씀)]자연조인(NATURAL JOIN) : 등가조인 방법 중 하나
--  => 동일한 타입과 이름을 가진 칼럼을 조인조건으로 이용하는 방법.
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE
NATURAL JOIN JOB;
--  운좋게 두개의 테이블에 일치하는 칼럼명이 "딱 한개" 만 존재 (JOB_CODE) -> 자연조건으로 사용시 알아서 JOB_CODE 가 매칭됨

--  조인 시 추가적인 조건 제시방법
--  직급이 대리인 사원들의 정보를 조회(사번, 사원명, 월급, 직급(직급코드가 아닌 직급명으로 조회)
...
--  4. 아래의 두 테이블을 참고해서 부서코드, 부서명, 지역코드, 지역면 조회
--  오라클 전용구문, ANSI구문 2가지로 표현
...
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
--  DEPT_CODE가 NULL인 사원은 조회되지 않는다. 즉, 모든 사원들의 정보가 출력되는게 아님

/*
    2. 포괄조인 , 외부조인(OUTER JOIN)
    테이블간의 JOIN시에 "일치하지 않는"행도 포함시켜서 조회가능
    단, 반드시 LEFT / RIGHT 를 지정해야 함. => 기준이 되는 테이블을 지정해야 한다.
    
    일치하는 행 + 기준이 되는 테이블을 기준으로는 일치하지 않는 행도 포함시켜서 조회
*/
--  LEFT OUTER JOIN : 두 테이블 중 왼쪽에 있는 테이블을 기준테이블로 삼음
--                    즉, 무조건 왼쪽에 기술된 테이블의 데이터는 무조건 조회되게끔 한다
--                    (조인하는 테이블과 일치하는 값이 없더라도)
--  ANSI 구문
SELECT EMP_NAME, SALARY, DEPT_TITLE
FROM EMPLOYEE LEFT /*OUTER*/ JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
--  EMPLOYEE 테이블을 기준으로 조회했기 때문에 EMPLOYEE에 존재하는 데이터는 뭐가 되었던 간에 다 조회 가능하게끔 해줌

--  ORCLE전용구문
SELECT EMP_NAME, SALARY, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+);
--  내가 기준으로 삼을 테이블의 반대 컬럼명에 (+)을 붙여준다

--  2) RIGHT OUTER JOIN : 두 테이블 중 오른편에 기술된 테이블을 기준으로 JOIN
--                        즉, 오른편에 기술된 테이블의 데이터는 무조건 조회되게 한다
...
--  [참고(잘안씀)]  3) FULL OUTER JOIN : 두 테이블이 가진 모든 행을 조회
--  일치하는 행들 + LEFTOUTER JOIN 기준 새롭게 추가된 행들 + RIGHT OUTER JOIN
...
/*
    ...
    각각 N개, M개의 행을 가진 테이블이 있다면 출력결과가 되는 행의 갯수는 N * M 행
...
FROM EMPLOYEE, DEPARTMENT
ORDER BY EMP_NAME ASC;-- 23인원 * 9개의 부서 -> 207개 행 조회
...
/*
    ...
    '=' 를 사용하지 않는 조인문 => 다른 비교 연산자를 써서 조인함(>,<,>=,<=,BETWEEN A AND B, LIKE, ...)
        지정한 칼럼값들이 일치하는 경우가 아니라 "범위"에 포함되는 경우 매칭에서 조회하겠다
    
    등가조인 -> '='으로 일치하는 경우만 조회
    비등가조인 -> '='가 아닌 다른 비교 연산자로 "범위"에 포함되는 경우를 조회
*/
...
/*
    ...
    -> 자체 조인의 경우 테이블에 반드시 별칭을 붙여줘야 한다.(별칭을 붙이면 서로 다른 테이블인 것 처럼 사용가능)
*/
--  사원의 사번, 사원명(EMP_NAME), 사수의 사번, 사수명(EMP_NAME)
--  ORACLE
