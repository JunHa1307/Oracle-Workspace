...
SELECT DEPT_CODE, SUM(SALARY)-- 4. SELECT 4번째로 실행
FROM EMPLOYEE       --  1. FROM절 먼저 실행
WHERE 1 = 1         --  2. WHERE절 두번째로 실행
GROUP BY DEPT_CODE  --  3. GROUPBY절 세번째로 실행
ORDER BY 2 DESC;    --  5. ORDER BY절 다섯번째로 실행

--  각 직급별 직급코드, 총 급여의 합, 사원수, 보너스를 받는 사원 수, 평균 급여, 최고 급여, 최소 급여
SELECT JOB_CODE "직급코드", SUM(SALARY)"총 급여의 합", COUNT(*)"사원수", COUNT(BONUS) "보너스를 받는 사원 수" , ROUND(AVG(SALARY)) "평균 급여" , MAX(SALARY) "최고 급여" , MIN(SALARY) "최소 급여"
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY 1;
...
FROM EMPLOYEE
--WHERE ROUND(AVG(SALARY)) >= 300000 에러남 . 문법상 그룹함수를 WHERE절에서 쓸 수가 없다
GROUP BY DEPT_CODE;
/*
    <HAVING>
...
--  각 직급별 급여 평균이 300만원 이상인 직급코드, 평균 급여, 사원 수, 최고급여, 최소급여
SELECT JOB_CODE, ROUND(AVG(SALARY)) "평균 급여", COUNT(*)"사원 수", MAX(SALARY) "최고 급여",MIN(SALARY) "최소 급여"
FROM EMPLOYEE
GROUP BY JOB_CODE
...
/*
    <SELECT 문 구조 및 실행순서>
    5. SELECT 조회하고자 하는 컬럼명 나열 / * / 리터럴 / 산술연산식 / 함수 / 별칭부여
    1. FROM 조회하고자 하는 테이블명 / 인라인  쿼리 / 가상 테이블(DUAL)
    2. WHERE 조건식 (그룹함수를 사용할 수 없음)
    3. GROUP BY 그룹 기준에 해당하는 칼럼명 / 함수식
    4. HAVING 그룹 함수식에 대한 조건식
    6. ORDER BY [정렬기준에 해당하는 컬럼명 / 별칭 / 컬럼의 순번] [ASC / DESC] 생략가능 [NULLS FIRST/ NULLS LAST]
*/
...
--  직급코드가 J6이거나 또는 부서코드가 D1인사원들을 조회 (사번, 사원명, 부서코드 , 직급코드) => UNION을 사용해서 데이터조회
...
--  UNION ALL : 여러 개의 쿼리결과를 더해서 보여주는 연산자.(중복제거 안함.)
--  직급코드가 J6이거나 또는 부서코드가 D1인사원들을 조회 (사번, 사원명, 부서코드 , 직급코드) => UNION을 사용해서 데이터조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE
FROM EMPLOYEE
WHERE DEPT_CODE = 'D1'
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE,JOB_CODE
FROM EMPLOYEE
WHERE JOB_CODE = 'J6' -- 정헌돈, 장쯔위, 하도운, 차태연, 전지연, 이태림
INTERSECT
SELECT EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE
FROM EMPLOYEE
WHERE DEPT_CODE = 'D1'; -- 방명수, 차태연, 전지연

--  4, MINUS : 차집합, 선행 쿼리결과에서 후행 쿼리 결과를 뺀 나머지
--  직급코드가 J6이거나 또는 부서코드가 D1인사원들을 조회 (사번, 사원명, 부서코드 , 직급코드) => UNION을 사용해서 데이터조회
