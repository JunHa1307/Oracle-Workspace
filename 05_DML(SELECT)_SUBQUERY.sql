/*
    SUBQUERY(서브쿼리)
    일반적으로 작성한 구문 = 쿼리
    하나의 주된 SQL(SELCT, CREATE, UPDATE, INSERT) 안에 포함된 또하나의 SELECT문
...
FROM EMPLOYEE;
--  2) 평균급여(3047663)보다 많은 급여를 받고있는 사원 조회
...
/*
    ...
    -   단일행 (단일 열) 서브쿼리 : 서브쿼리를 수행한 결과값이 오직 1개일 때(한 칸의 컬럼값으로 나올 때)
    -   다중행 (단일 열) 서브쿼리 : 서브쿼리를 수행한 결과값이 여러 행일 때
    -   (단일 행) 다중열 서브쿼리 : 서브쿼리를 수행한 결과값이 여러 열일 때
...
--  부서별로 급여 합이 가장 큰 부서 하나만 조회, 부서코드, 부서명, 급여 합
...
/*
    ...
    -   IN (10, 20, 30, ...) 서브 쿼리 : 여러 개의 결과값 중 하나라도 일치하는 것이 있다면 / 일치하는 것이 없다면 NOT IN
    -   > ANY(10, 20, 30, ...) : 여러 개의 결과값 중에서 하나라도 클 경우 즉, 여러 개의 결과값중에서 가장 작은 값보다 클 경우 TRUE
    -   < ANY(10, 20, 30, ...) : 여러 개의 결과값 중에서 하나라도 작을 경우 즉, 여러 개의 결과값중에서 가장 큰 값보다 작을 경우 TRUE
    
    -   > ALL : 여러 개의 결과값의 모든 값보다 클 경우 TRUE
            즉, 여러 개의 결과값중에서 가장 큰 값보다 클 경우 TRUE
    -   < ALL : 여러 개의 결과값의 모든 값보다 작을 0경우 TRUE
            즉, 여러 개의 결과값중에서 가장 작은 값보다 작을 경우 TRUE
*/
...
--  선동일 또는 유재식과 같은 부서인 사원들을 조회,사원명,부서코드,급여
...
/*
    ...
    서브쿼리 조회 결과가 값은 한 행이지만, 나열된 컬럼의 갯수가 여러 개인 경우 ..
*/
--  하이유 사원과 같은 부서코드, 같은 직급코드에 해당되는 사원들 조회(사원명, 부서코드, 직급코드, 고용일)
--  1) 하이유 사원의 부서코드와 직급코드 먼저 조회 => 단일 행 다중열(DEPT_CODE, JOB_CODE)
...
--  (비교대상칼럼1, 비교대상칼럼2) = (비교할 값 1, 비교할 값2) -> 리터럴 X ,서브쿼리로 제시
SELECT EMP_NAME, DEPT_CODE, JOB_CODE, HIRE_DATE
FROM EMPLOYEE
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE
                               FROM EMPLOYEE
                               WHERE EMP_NAME = '하이유');
--  박나라 사원과 같은 직급코드. 같은 사수 사번을 가진 사원들의 사번, 이름, 직급 코드, 사수 사번 조회
--  다중 열 서브쿼리로 작성하기 
...
--  부서가 없는 사원의 경우 없음이라는 부서로 출력 되도록 하기
SELECT EMP_ID, EMP_NAME, NVL(DEPT_CODE,'없음'), SALARY
FROM EMPLOYEE
WHERE (SALARY,NVL(DEPT_CODE,'없음')) IN (SELECT MAX(SALARY),NVL(DEPT_CODE,'없음') 
                                        FROM EMPLOYEE 
                                        GROUP BY DEPT_CODE)
...
ORDER BY SALARY DESC
WHERE ROWNUM <= 5;
--  에러원인 : ORDER BY로 정렬하기 전에 이미 ROWNUM의 순번이 매겨져 있기 때문
--  해결방법 : ORDER BY로 이미 정렬할 테이블을 가지고 ROWNUM순번을 매기면 됨
...
SELECT ROWNUM, S.* --"ROUND(AVG(SALARY))"  ,  AS SALARY
FROM (
        SELECT DEPT_CODE, ROUND(AVG(SALARY))
        FROM EMPLOYEE
        GROUP BY DEPT_CODE
...
--  선 정렬 후 순위매기기를 해야한다 => 인라인 뷰로 ORDER BY절을 만들고 그 후에 메인쿼리에서 순번을 붙임
...
/*
    4.  순위 메기는 함수(WINDOW FUNCTION)
    RANK() OVER(정렬기준)
    DENSE_RANK() OVER (정렬기준)
    
    - RANK() OVER(정렬기준) : 공동1위가 3명이라고 한다면 그 다음순위는 4위로
    - DENSE_RANK OVER(정렬기준) : 공동1위가 3명이면 그 다음 순위는 2위로
    
    정렬기준 : ORDER BY절 (정렬기준 컬럼이름 ,오름차순/내림차순), NULLSFIRST나 NULLSLAST는 기술불가
    
    오직 SELECT절에서만 기술 가능
*/
--  사원들의 급여가 높은 순서대로 매기기, 사원명, 급여, 순위 조회 : RANK OVER
SELECT EMP_NAME, SALARY, RANK() OVER(ORDER BY SALARY DESC) 순위
FROM EMPLOYEE;--    공동 19위 2명, 그 다음순위는 21위
...
--  윈도우 함수를 WHERE절에서 기술할 수 없기 때문에 잘 쓰이지 않음 <- 인라인 뷰에 넣어서 써도 되긴한데 ROWNUM을 쓰면 됨
