...
--  DEPT_CODE != 'D9';
--  DEPT_CODE ^= 'D9';
    DEPT_CODE <> 'D9';-- 이게 표준이긴 한데 다른걸로 써도 무방
...
SELECT EMP_NAME, SALARY, SALARY * 12 AS "SALARY(YEAR)", HIRE_DATE
FROM EMPLOYEE
WHERE
    (SALARY * 12) >= 50000000;
--  "SALARY(YEAR)" >= 50000000; <- 이건 안됨 FROM -> WHERE -> SELECT 순으로 진행되기 때문에 
-- SELECT 절에서 부여한 별칭은 WHERE절에서 사용할 수 없다

SELECT C.*
FROM (
    SELECT EMP_NAME, SALARY, SALARY * 12 AS "연봉", HIRE_DATE
    FROM EMPLOYEE
    )C-- <- 괄호 안 테이블의 별칭(나중에 안쓰면 오류날수도 있음)
WHERE
    C.연봉 >= 50000000;
...
-- 급여가 350만원 미만이고, 600 만원 초과인 사원들의 이름, 사번, 급여, 직급코드 조회
...
-- 전화번호 네 번째 자리가 9로 시작하는 사원들의 사번,사원명, 전화번호, 이메일 조회
...
-- 부서배치는 받지 않았지만 보너스는 받는 사원의 모든칼럼 조회
SELECT *
FROM EMPLOYEE
...
/*
    < IN >
    비교대상 칼럼 값에 내가 제시한 목록들 중 일치하는 값이 있는지 판단.
...
FROM EMPLOYEE
WHERE 
    JOB_CODE NOT IN ('J1','J3','J4');

/*
    <연결 연산자 ||>
    여러 칼럼값들을 마치 하나의 컬럼인 것 처럼 연결시켜주는 연산자
...
/*
    ...
    <ORDER BY>
    SELECT문 가장 마지막에 기입하는 구문뿐 만 아니라 가장 마지막에 실행되는 구문
...
SELECT *
FROM EMPLOYEE
--  ORDER BY BONUS; ASC(오름차순) AND NULLSLAST <- 이 두개가 기본값
--  ORDER BY BONUS ASC NULLS FIRST;
--  ORDER BY BONUS DESC;<- NULLSFIRST 가 기본값
ORDER BY BONUS DESC, SALARY ASC;
--> 첫 번째로 제시한 정렬기중 컬럼값이 일치할 경우 두 번째 정렬기준을 가지고 다시 정렬
...
ORDER BY 3;--칼럼의 순서로 정렬

--> ORDER BY 절은 숫자 뿐만이 아니라 문자열, 날짜에 대해서도 정렬 가능하다.