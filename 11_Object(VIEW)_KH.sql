...
--  접속계정 변경(SYSTEM)후 권한 부여
GRANT CREATE VIEW TO KH;

SELECT * FROM V_EMPLOYEEALL;

--  VIEW FROM에서 테이블인것처럼 사용가능
...
--  뷰 특징
--  1. 컬럼에 대해 산술 연산 및 함수의 실행결과를 포함할 수 있다. 단, 반드시 별칭을 부여해야함
...
--  DECODE함수를 이용해서 사원의 이름, 사번, 직업명, 성별, 근속 년수를 VIEW로 저장
...
SELECT * FROM V_EMP_JOB WHERE 근속년수 >= 20;

/*
    INSERT, UPDATE, DELETE를 수행할 때 -> 가능하지만 실제 테이블에 담겨 있는 값이 변경됨
    만약 내가 UPDATE하고자하는 칼럼이 가상 칼럼이라면 DML이 불가능함
*/
...
/*
    ...
    2. 뷰에 포함되지 않은 컬럼 중 베이스가 되는 컬럼에 NOT NULL 제약조건이 설정되어 있을 때
    3. 산술연산으로 구성된 칼럼(가상칼럼)
    4. 그룹함수, GROUP BY절이 포함된 VIEW
    5. DISTINCT를 포함하고 있을 경우
    6. JOIN을 통해 여러 테이블을 연결하고 있을 경우
*/

--  VIEW 옵션들
--  1. OR REPLACE 옵션 -> VIEW가 이미 존재한다면 현재 실행된 서브쿼리로 뷰를 바꿔주는 역할

--  2. FORCE/ MPFORCE 옵션 : 실제 테이블이 없어도 VIEW를 먼저 생성할 수 있게 해주는 옵션
