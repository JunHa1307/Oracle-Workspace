...
FROM DUAL; -- 가상테이블(DUMMY TABLE) : 산술연산이나 가상칼럼값등 한 번만 출력하고 싶을 때 사용하는 테이블
...
SELECT INSTR('AABAACAABBAA','B',-1) FROM DUAL;
--  뒤에서부터 B를 찾아서 첫번째로 찾는 B의 위치를 반환해줌.(숫자는 앞에서 부터 샌다)

SELECT INSTR('AABAACAABBAA','B',-1,2) FROM DUAL;
--  뒤에서부터 B를 찾아서 두번째로 찾는 B의 위치를 반환.
...
/*
    ...
    추출할 문자갯수 생략가능(생략 시 문자열 끝까지 추출하겠다는 뜻)
    처음위치는 음수로도 제시 가능 : 뒤에서부터 N번째 위치로부터 문자를 추출하겠다 라는 뜻
*/
...
SELECT RPAD(EMAIL,20,'@'),LENGTH(EMAIL) FROM EMPLOYEE;
...
SELECT LTRIM('0001230456000','0') FROM DUAL;

SELECT RTRIM('0001230456000','0') FROM DUAL;

SELECT LTRIM('12123KH123','123') FROM DUAL;

SELECT LTRIM('1313KH123','123') FROM DUAL;
--  제거시키고자 하는 문자열을 통으로 지워주는게 아니라 문자 하나하나 검사를 하면서 현재문자가 지우고자하는 문자에 있다면 지워줌.
...
SELECT CONCAT (CONCAT('가나다','라마바사') , '아')FROM DUAL; -- 두가지 연결연산자 사용 안됨
...
/*
    ...
    반올림할 위치 : 소수점 기준으로 아래 N번째 수에서 반올림 하겠다.
                    생략가능(기본값은 0, 소숫점 첫번째 자리에서 반올림을 하겠다 == 소수점이 0개다)
*/
...
/*
    ...
    - FLOOR(버림처리할 숫자) : 소숫점아래의 수를 무조건 버림처리하는 함수
    
    반환형은 NUMBER타입
*/

SELECT SEIL(123.000000000000000001) FROM DUAL;

SELECT FLOOR(123.999999999999999999) FROM DUAL;
...
/*
    ...
    DATE타입 : 년도, 월 ,일, 시, 분, 초를 다 포함하고 있는 자료형
*/
...
--  1. NEXT_DAY(DATE, 요일(문자/숫자)): 특정날짜에서 가장 가까운 요일을 찾아 그 날짜를 반환
...
ALTER SESSION SET NLS_LANGUAGE = AMRTICAN;
...
/*
    ...
    : 숫자형 또는 날짜형 데이터를 문자형 타입으로 반환(포멧 맞춰서)
*/
--  숫자를 문자열로
...
SELECT TO_CHAR(TO_DATE('220806','YYMMDD'),'FMYYMMDD')
FROM DUAL;
--  0제거 (22/8/6)
...
-- MON : 몇 '월' 형식, DY : 요일을 알려주는데 금요일에서 요일을 뺌('금') DAY : '금요일'
...
--  YY와 RR의 차이점
--  R이 뜻하는 단어 : ROUND(반올림)
--  YY : 앞자리에 무조건 20이 붙음 => (20)22
--  RR : 50년 기준 작으면 20이 붙음, 크면 19가 붙음 => (19)89 
 
--  2022년 11월 4일 (금) 포멧으로 적용하기
...
FROM DUAL; -- 기본포멧 YY/MM/DD로 변환

SELECT TO_DATE(000101) -- 정수값중에 0으로시작하는 숫자는 없기때문에 에러발생
FROM DUAL;
...
FROM DUAL; -- 2098년도
--  TO_DATE()함수를 이용해서 DATE형식으로 반환시 두자리 년도에 대해 YY형시을 적용시키면 무조건 현재세기(20)를 붙여줌
...
FROM DUAL; -- 1998년
-- 두자리 년도에 대해 RR포멧을 적용시켰을 경우 => 50이상이면 이전세기, 50미만이면 현재세기(반올림)
...
/*
    NULL 처리 함수
    NVL(컬럼명, 해당 컬럼값이 NULL일경우 반환할 반환 값)
    -- 해당 칼럼값이 존재할 경우(NOT NULL일 경우) 기존의 컬럼값을 반환,
    -- 해당 컬럼값이 존재하지 않을 경우 (NULL일 경우) 내가 제시한 특정 값을 반환
*/
...
--  보너스 포함 연봉 조회. (SALARY + SALARY * BONUS) 8 12
SELECT EMP_NAME, (SALARY + (SALARY * NVL(BONUS, 0) )) * 12 AS "보너스가 포함된 연봉"
FROM EMPLOYEE;

--  사원명, 부서코드 (부서코드가 없는 경우 '없음') 조회
SELECT EMP_NAME, NVL(DEPT_CODE, '없음') AS DEPT_CODE
FROM EMPLOYEE;

--  NVL2(컬럼명, 결과값1, 결과값2)
--  해당칼럼에 값이 존재할 경우 결과값 1 반환
--  해당칼럼에 값이 NULL일 경우 결과값 2 반환
...
SELECT EMP_NAME, JOB_CODE, SALARY AS "변경 전 급여",DECODE(
            JOB_CODE,'J7',SALARY * 1.2 ,
            'J6',SALARY * 1.15,
            'J5',SALARY*1.5 ,
            SALARY * 1.1 )AS "변경 전 급여"
FROM EMPLOYEE;
...
/*
    N개의 값을 읽어서 1개의 결과를 반환(하나의 그룹별고 함수 실행 결과 반환)
*/

--  1. SUM(숫자타입의 컬럼) : 해당컬럼겂들의 총 합계를 반환
SELECT SUM(SALARY)
FROM EMPLOYEE;
...
--  2. AVG(숫자타입컬럼) : 행당 컬럼값들의 평균을 구해서 반환
SELECT ROUND(AVG(SALARY))
FROM EMPLOYEE;

--  3. MIN(ANY타입) : 해당칼럼값들 중 가장 작은 값 반환
SELECT MIN(SALARY),MIN(EMP_NAME),MIN(EMAIL),MIN(HIRE_DATE)
FROM EMPLOYEE;

--  4. MAX(ANY타입) : 해당컬럼값들 중 가장 큰값을 반환
SELECT MAX(SALARY),MAX(EMP_NAME),MAX(EMAIL),MAX(HIRE_DATE)
FROM EMPLOYEE;
--  MAX함수의 경우 내림차순으로 정렬 시 가장 위쪽의 값을 보여준다
